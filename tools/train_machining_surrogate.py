...
from larrak2.gear.picogk_adapter import _cache_key
import hashlib
...
def train_surrogate():
    ...
    process = ManufacturingProcessParams(
        kerf_mm=0.2, overcut_mm=0.05, min_ligament_mm=0.25 # Matches DOE
    )
    ...
    for dur in durations:
        for amp in amps:
            ...
            for shape_name, ratio_profile in candidates:
                ...
                key = make_cache_key(
                    theta, r_planet, 
                    process.kerf_mm, process.overcut_mm, 
                    0.0, process.min_ligament_mm, 0.1
                )
                ...
                if key in cache:
                    ...
                    x_vec = [
                        ...
                        (amp + 1.5) / 5.5,
                        # Shape OneHot
                        *[1.0 if i == shape_map[shape_name] else 0.0 for i in range(len(PROFILE_NAMES))]
                    ]
